...
export async function loadQrSettings(db: any): Promise<{ mode: "off" | "flag" | "block"; secret: string }> {
  const { data } = await db.from("app_settings").select("key, value").in("key", ["qr_login_mode", "qr_secret"]);
