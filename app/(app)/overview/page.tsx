  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        <StatCard label="Actual Money Drop" value={money(cashTotals.cash_sales)} hint={`${cashReports.length} report(s)`} />
              ...
              return (
                <div key={report.id} className={`rounded-lg border p-3 text-sm ${flag === "Clear" ? "border-slate-200 dark:border-slate-800" : "border-red-200 bg-red-50/60 dark:border-red-900 dark:bg-red-950/20"}`}>
                  ...
                  <div className="mt-2 grid gap-2 text-xs text-slate-500 md:grid-cols-4">
                    ...
                    <span>Cash: Hike {money(Number(report.expected_cash ?? 0))} / actual drop {money(Number(report.counted_cash ?? report.cash_sales ?? 0))}</span>
                  ...
                  {(report.notes || report.expense_notes) && (
