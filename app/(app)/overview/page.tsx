...
export default async function OverviewPage({ searchParams }: { searchParams: { date?: string } }) {
  ...
  const [instRes, attRes, incRes, deptsRes, followRes] = await Promise.all([
    supabase.from("checklist_instances")
      .select(`id, profile_id, department_id, status,
        ...
        checklist_tasks(id, title, status, priority, tags, is_overdue, employee_remarks, supervisor_remarks, completed_at, duration_minutes)`)
      .eq("work_date", date),
